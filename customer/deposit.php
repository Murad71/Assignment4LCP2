...
$currentBalance = isset($balances[$user_email]) ? $balances[$user_email] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount !== false && $amount > 0) {
        ...
        if (!isset($balances[$user_email])) {
            $balances[$user_email] = 0;
        }
        $balances[$user_email] += $amount;
                                ...
                                <div>
                                    <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                                        ...
                                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                                            <span class="font-medium leading-none text-emerald-700"><?= substr($user_name, 0, 2) ?></span>
                                        </span>
                                    </button>
                                </div>
                                <!-- Dropdown menu -->
                    ...
                    <div class="border-t border-emerald-700 pb-3 pt-4">
                        <div class="flex items-center px-5">
                            <div class="flex-shrink-0">
                                <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                                    <span class="font-medium leading-none text-emerald-700"><?= substr($user['name'], 0, 2) ?></span>
                                </span>
                            </div>
                            <div class="ml-3">
                            ...
                            <div class="mt-4 text-sm text-gray-500">
                                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                                    ...
                                    <?php if (isset($successMessage)): ?>
                                        <div class="mt-4 text-green-600"><?php echo htmlspecialchars($successMessage); ?></div>
                                    <?php endif; ?>
                                    <?php if (isset($errorMessage)): ?>
                                        <div class="mt-4 text-red-600"><?php echo htmlspecialchars($errorMessage); ?></div>
                                    <?php endif; ?>
                                </form>
                            </div>
                        </div>
