...
$allTransactionsData = file_exists($transactions_file) ? json_decode(file_get_contents($transactions_file), true) : [];

$file = 'balances.json';
$balances = file_exists($file) ? json_decode(file_get_contents($file), true) : [];
$currentBalance = isset($balances[$user['email']]) ? $balances[$user['email']] : 0;


$transactions = $allTransactionsData[$user['email']] ?? [];
            ...
            <?php foreach ($transactions as $transaction): ?>
                <tr>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                        <?php echo isset($transaction['from']) ? htmlspecialchars($transaction['from']) : htmlspecialchars($transaction['to']); ?>
                    </td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-0">
                        <?php echo htmlspecialchars($transaction['email']); ?>
                    </td>
                    <td class="whitespace-nowrap px-2 py-4 text-sm font-medium <?php echo isset($transaction['to']) ? 'text-red-600' : 'text-emerald-600'; ?>">
                        <?php echo (isset($transaction['to']) ? '-' : '+') . '$' . number_format($transaction['amount'], 2); ?>
                    </td>
                    <td class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                        <?php echo date('d M Y, h:i A', strtotime($transaction['date'])); ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
