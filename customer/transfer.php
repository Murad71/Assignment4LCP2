...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $receiver_email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);

    if ($amount !== false && $amount > 0 && !empty($receiver_email)) {

  
        $receiverExists = false;
        $receiverName= "";
        foreach ($allUsers as $receiverUser) {
            if ($receiverUser['email'] === $receiver_email) {
                $receiverExists = true;
        ...
        if (!$receiverExists) {
        ...
        } elseif ($balances[$_SESSION['user_email']] < $amount) {
            $errorMessage = "Insufficient balance.";
        } else {
           
            $balances[$_SESSION['user_email']] -= $amount;

      
            if (!isset($balances[$receiver_email])) {
                $balances[$receiver_email] = 0;
            }
            $balances[$receiver_email] += $amount;
            ...
            if (!isset($transactions[$_SESSION['user_email']])) {
                $transactions[$_SESSION['user_email']] = [];
            }
        ...
        $errorMessage = "Invalid amount or receiver email.";
    }
}

$currentBalance = isset($balances[$_SESSION['user_email']]) ? $balances[$_SESSION['user_email']] : 0;
