...
if (isset($_GET['email'])) {
  ...
  $transactionsFile = '../customer/transactions.json';
  $transactionsData = file_exists($transactionsFile) ? json_decode(file_get_contents($transactionsFile), true) : [];
  ...
  if(empty($transactionsData[$decodedEmail]))
  {
    header("Location: customers.php");
  }

  foreach($transactionsData as $userEmail => $userTransactions)
  {
              ...
              <div class="mt-8 flow-root">
                ...
                <tr>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                        <?php echo isset($userTransaction['from']) ? htmlspecialchars($userTransaction['from']) : htmlspecialchars($userTransaction['to']); ?>
                    </td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-0">
                        <?php echo htmlspecialchars($userTransaction['email']); ?>
                    </td>
                    <td class="whitespace-nowrap px-2 py-4 text-sm font-medium <?php echo isset($userTransaction['to']) ? 'text-red-600' : 'text-emerald-600'; ?>">
                        <?php echo (isset($userTransaction['to']) ? '-' : '+') . '$' . number_format($userTransaction['amount'], 2); ?>
                    </td>
                    <td class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                        <?php echo date('d M Y, h:i A', strtotime($userTransaction['date'])); ?>
                    </td>
                </tr>
            <?php endforeach; ?>
                      </tbody>
